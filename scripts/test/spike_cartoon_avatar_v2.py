...
OUT = os.environ.get("SPIKE_OUT", f"{ROOT}/scripts/test/alif_cartoon_v2.mp4")
...
GHUTRA = (248, 249, 250)
GHUTRA_SHADE = (221, 224, 227)
AGAL = (35, 33, 32)
AGAL_HILITE = (78, 76, 76)
TARBOOSH = (35, 33, 32)  # front cord + tassel hanging off the agal/ghutra
SKIN = (109, 156, 217)
...
def draw_body(canvas, pose_px, w, h, scale_w=None):
    ...
    # Head + ghutra/agal/tarboosh. Everything here is anchored off head_c
    # and neck (both purely position-driven, no independent motion of
    # their own) so the cloth reads as rigidly attached to the head and
    # only moves when the head/neck does - it doesn't flap or animate on
    # its own. Face proportions: an oval (taller than wide) rather than
    # the old plain circle, closer to a real jaw/forehead shape.
    # Face is deliberately smaller than the head/ghutra silhouette so the
    # ghutra actually reads as a hood framing the face on both sides,
    # rather than being fully hidden behind a face that covers the whole
    # head (the previous version's actual bug).
    face_rx = int(head_r * 0.52)
    face_ry = int(head_r * 0.68)
    face_c = (head_c[0], head_c[1] + int(head_r * 0.16))

    # Ghutra drape: a rounded hood (arc across the crown, not a pointed
    # triangle - a hard point read as a party hat) that flares out past
    # the temples and falls to both shoulders. Drawn behind the face.
    dome_c = (head_c[0], head_c[1] - int(head_r * 0.02))
    dome_rx = int(face_rx * 1.45)
    dome_ry = int(head_r * 0.98)
    dome_arc = cv2.ellipse2Poly(dome_c, (dome_rx, dome_ry), 0, 180, 360, 8)
    ghutra_l_shoulder_in = (head_c[0] - dome_rx, head_c[1] - int(head_r * 0.05))
    ghutra_r_shoulder_in = (head_c[0] + dome_rx, head_c[1] - int(head_r * 0.05))
    ghutra_poly = np.vstack([dome_arc, [ipt(r_sh), ipt(l_sh)]]).astype(np.int32)
    cv2.fillConvexPoly(canvas, ghutra_poly, GHUTRA, cv2.LINE_AA)
    # soft fold shading down each side, same cheap-depth trick as the kandura
    cv2.line(canvas, ipt(ghutra_l_shoulder_in), ipt((l_sh[0], l_sh[1] - torso_h * 0.05)), GHUTRA_SHADE, 3, cv2.LINE_AA)
    cv2.line(canvas, ipt(ghutra_r_shoulder_in), ipt((r_sh[0], r_sh[1] - torso_h * 0.05)), GHUTRA_SHADE, 3, cv2.LINE_AA)
    cv2.polylines(canvas, [ghutra_poly], True, KANDURA_LINE, 2, cv2.LINE_AA)

    # Face (on top of the ghutra drape, ghutra now visible framing both sides)
    cv2.ellipse(canvas, face_c, (face_rx, face_ry), 0, 0, 360, SKIN, -1, cv2.LINE_AA)
    cv2.ellipse(canvas, face_c, (face_rx, face_ry), 0, 0, 360, SKIN_LINE, 2, cv2.LINE_AA)

    # Agal: a rounded cord band hugging the forehead/hairline directly
    # (not floating above the head) - drawn as a thin arc-like band that
    # follows the top curve of the face rather than a flat wide ellipse.
    agal_cy = face_c[1] - int(face_ry * 0.82)
    agal_half_w = int(face_rx * 1.12)
    agal_h = max(6, int(head_r * 0.13))
    cv2.ellipse(canvas, (head_c[0], agal_cy), (agal_half_w, agal_h), 0, 15, 165, AGAL, -1, cv2.LINE_AA)
    cv2.ellipse(canvas, (head_c[0], agal_cy - int(agal_h * 0.35)), (int(agal_half_w * 0.82), int(agal_h * 0.5)), 0, 15, 165, AGAL_HILITE, -1, cv2.LINE_AA)
    cv2.ellipse(canvas, (head_c[0], agal_cy), (agal_half_w, agal_h), 0, 15, 165, KANDURA_LINE, 1, cv2.LINE_AA)

    # Tarboosh: the cord + tassel hanging down the front of the ghutra,
    # over the chest. Starts below the chin (not over the face) and is
    # anchored to neck, so it tracks the head the same way the ghutra does.
    tar_top = (head_c[0], face_c[1] + int(face_ry * 0.98))
    tar_bot = (int(neck[0]), int(neck[1] + torso_h * 0.55))
    cv2.line(canvas, ipt(tar_top), ipt(tar_bot), TARBOOSH, 3, cv2.LINE_AA)
    tassel_r = max(4, int(scale_w * 0.05))
    cv2.circle(canvas, ipt(tar_bot), tassel_r, TARBOOSH, -1, cv2.LINE_AA)
    cv2.line(canvas, ipt(tar_bot), ipt((tar_bot[0], tar_bot[1] + tassel_r * 2)), TARBOOSH, 2, cv2.LINE_AA)
