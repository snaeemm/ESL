...
**Update (same day, later commits):** the hero job was switched again, from `5e7a072a1a49` (described in the "Real job used" section below) to `013fbd2aa3f0` ("My Family and My Day at School") — **100% verified institutional (ZHO) sign coverage, zero fingerspelling, zero review-required items** (86.4% ZHO institutional + 13.6% ESL Zayed supplementary, 0% fallback), vs. `5e7a072a1a49`'s 75%/8.3%/16.7% split. `demo/MOE_Website_Walkthrough_Demo.mp4` was rebuilt against this new hero, with a hand-jitter fix applied in `scripts/spike_render_captioned_lesson.py` (the avatar-rescale fix amplified residual hand jitter; a second light EMA smoothing pass after the rescale brought it back down, verified against the job's own exported motion data). `demo/MOE_Prototype_Lesson_Demo.mp4` (the raw generated episode, no UI chrome) was updated to the same re-rendered `013fbd2aa3f0` output for consistency between the two demo videos. The specific job ID, screenshot URLs, and per-shot coverage numbers in the sections below now describe the superseded `5e7a072a1a49` run and are kept as a historical record of how the walkthrough was originally built, not the current video's exact content.
